...
| Importancia | Alta |
| Estado | Aprobado |
| Comentarios | El mecanismo concreto de gestión de la sesión (emisión y validación de tokens, almacenamiento en cookies) es una decisión de diseño. La protección frente a intentos de acceso reiterados se aborda en los requisitos no funcionales de seguridad. |
...
| Importancia | Baja |
| Estado | Aprobado |
| Comentarios | Solo se pueden eliminar consultas del propio usuario. La eliminación es definitiva e impide cualquier recuperación posterior, en coherencia con el derecho de supresión del RGPD. |
...
| Administrador | Primario | Usuario autenticado responsable del gobierno de la plataforma. | Todas las capacidades del usuario autenticado, más la gestión de usuarios y la supervisión de la actividad (CU-031 a CU-033). |
...
Además de las métricas numéricas, el laboratorio debe permitir la inspección visual de las explicaciones de cada modelo. El sistema debe mostrar la galería de mapas de calor generados por el análisis XAI cualitativo sobre imágenes de ejemplo del dataset de entrenamiento, lo que permite comprobar si el modelo se fija en las regiones pulmonares relevantes o si su atención se dispersa hacia artefactos de la imagen. Esta inspección visual complementa las métricas cuantitativas y constituye una herramienta valiosa para evaluar la calidad de las explicaciones. El caso de uso CU-021 especifica esta visualización y extiende la consulta de resultados del modelo (CU-020).
...
En determinadas circunstancias, el usuario puede necesitar detener un trabajo que ha encolado por error o que ya no le interesa, como un diagnóstico solicitado por equivocación o un entrenamiento que no desea esperar. El sistema debe permitir cancelar un trabajo pendiente, de modo que no llegue a ejecutarse, y, cuando resulte técnicamente posible, interrumpir un trabajo en ejecución, de modo que un entrenamiento lanzado por error no monopolice la GPU sin posibilidad de detenerse. Si un trabajo en ejecución no puede interrumpirse de forma segura, el sistema informa de esa limitación. El caso de uso CU-035 especifica esta operación.
...
El entrenamiento de modelos compite por los recursos computacionales de la estación de trabajo, que cuenta con una única GPU. El sistema limita el número de entrenamientos que se ejecutan simultáneamente y el número de trabajos de entrenamiento que pueden permanecer encolados. Cuando el usuario lanza un experimento y se supera el límite de ejecución concurrente, el trabajo queda encolado y espera a que haya recursos disponibles; el usuario puede comprobarlo desde el panel de la cola de trabajos. El caso de uso CU-039 especifica este comportamiento y pertenece al módulo de laboratorio MLOps.
