...
const _origTrainingChangeLang = window.changeLanguage || function(){};
window.changeLanguage = function() {
    _origTrainingChangeLang();
    const t_ = window.t || (k=>k);
    const ids = ['ui-nav-diag', 'ui-nav-lab', 'ui-history-title', 'ui-no-history', 'ui-logout', 'ui-admin-btn', 'ui-queue-title', 'ui-queue-empty'];
    const keys = ['navDiag', 'navLab', 'historyTitle', 'noHistory', 'logout', 'adminBtn', 'queueTitle', 'queueEmpty'];
    ...
    if (uiTitle) uiTitle.innerText = t_('trainConfigAssistant');
};
...
async function loadSidebarModels() {
    ...
    try {
        ...
        if (data.sessions && data.sessions.length > 0) {
            ...
            data.sessions.forEach(session => {
                ...
                <div class="mb-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm relative group/session cursor-pointer" onclick="viewSessionResults('${session.session_id}')">
                    ...
                    <div class="absolute right-12 top-3.5 opacity-0 group-hover/session:opacity-100 transition-opacity flex items-center gap-1 bg-white dark:bg-gray-800 shadow-sm rounded-md p-1 border border-gray-200 dark:border-gray-600 z-10" onclick="event.stopPropagation()">
                        <button onclick="renameSession(event, '${session.session_id}')" class="w-6 h-6 flex items-center justify-center rounded bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors" title="${t_('trainRename')}"><i class="fa-solid fa-pencil text-[10px]"></i></button>
                        <button onclick="deleteSession(event, '${session.session_id}')" class="w-6 h-6 flex items-center justify-center rounded bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors" title="${t_('trainRename')}"><i class="fa-solid fa-trash text-[10px]"></i></button>
                    </div>
            ...
            list.innerHTML = `<div class="text-center mt-10"><i class="fa-solid fa-box-open text-3xl text-gray-300 dark:text-gray-600 mb-3"></i><p class="text-sm text-gray-400 dark:text-gray-500 font-medium">${t_('trainNoSessions')}</p></div>`;
        }
    } catch (e) {}
}
...
async function handleChatSubmit(event) {
    ...
    loadingDiv.innerHTML = `<div class="chat-bubble-ai px-4 py-3 rounded-2xl text-sm shadow-sm flex items-center gap-2"><i class="fa-solid fa-circle-notch fa-spin"></i> ${t_('trainThinking')}</div>`;
    ...
    try {
        ...
        if (data.response.includes('"ready": true') || data.response.includes("'ready': true")) {
            try {
                ...
                if (jsonMatch) {
                    ...
                    appendChatMessage('ai', `${t_('trainReady')} <b>${config.models}</b>.`);
                    startTrainingFromChat(config);
                }
            } catch (e) {
                appendChatMessage('ai', t_('trainParseError'));
            }
        } else {
        ...
        document.getElementById('chat-loading').remove();
        appendChatMessage('ai', t_('trainConnectionError'));
    }
...
function customPrompt(msg, def) {
    ...
    return new Promise(resolve => {
        ...
        box.innerHTML = `<p style="margin:0 0 12px;font-weight:700;font-size:14px">${msg}</p><input id="cp-input" value="${def}" style="width:100%;padding:8px 12px;border:1px solid #ccc;border-radius:8px;font-size:14px;box-sizing:border-box"><div style="display:flex;gap:8px;justify-content:flex-end;margin-top:16px"><button id="cp-cancel" style="padding:8px 16px;border-radius:8px;border:1px solid #ccc;background:white;cursor:pointer">${t_('trainCancel')}</button><button id="cp-ok" style="padding:8px 16px;border-radius:8px;border:none;background:#2563eb;color:white;cursor:pointer">${t_('trainConfirm')}</button></div>`;
...
async function renameSession(event, oldId) {
    ...
    const newName = await customPrompt(t_('trainRenamePrompt'), oldId);
...
async function deleteSession(event, sessionId) {
    const t_ = window.t || (k=>k);
    if(event) event.stopPropagation();
    if(!confirm(t_('trainDeleteConfirm').replace('{session}', sessionId))) return;
    try {
...
function cloneSessionConfig() {
    ...
    const autoMessage = t_('reuseMsg')
        .replace('{path}', currentSessionConfig.dataset_path)
        .replace('{models}', modelsStr)
        .replace('{epochs}', currentSessionConfig.epochs || 20)
        .replace('{batch}', currentSessionConfig.batch_size || 32)
        .replace('{lr}', currentSessionConfig.learning_rate || 0.001);
    ...
    try {
        ...
        if(data.status === "success") {
            data.ranking.forEach((r, i) => {
                let rankIcon = i === 0 ? '\u{1F3C6}' : (i === 1 ? '\u{1F948}' : (i === 2 ? '\u{1F949}' : `${i+1}º`));
        ...
        } else { tbody.innerHTML = `<tr><td colspan="4" class="p-4 text-center text-gray-500">${t_('trainNoResults')}</td></tr>`; img.classList.add('hidden'); err.classList.add('hidden'); }
        try {
...
async function downloadPDFReport() {
    ...
    btn.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> ${t_('trainGenerating')}`;
    btn.disabled = true;
