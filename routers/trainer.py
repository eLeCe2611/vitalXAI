...
async def chat_route(session_id: str = Form(...), message: str = Form(...)):
...
async def start_training(background_tasks: BackgroundTasks, model_names: str = Form(...), dataset_path: str = Form(...), epochs: int = Form(...), batch_size: int = Form(...), learning_rate: float = Form(...)):
    if not dataset_path or not os.path.exists(dataset_path):
        return JSONResponse(status_code=400, content={"status": "error", "message": "La ruta no existe."})
    session_id = mlops_engine.create_training_session(model_names, dataset_path, epochs, batch_size, learning_rate)
...
async def get_trained_sessions():
    return JSONResponse(content={"status": "success", "sessions": mlops_engine.get_trained_sessions()})
...
async def pdf_report_route(session_id: str):
